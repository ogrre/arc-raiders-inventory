    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <form method="GET" action="{{ route('inventory.add') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    @foreach ($items as $item)
                        @php
                            ...
                            $currentQuantity = $userInventory[$item['id']] ?? 0;
                        @endphp

                        <div class="bg-white border-2 {{ $borderColor }} rounded-lg overflow-hidden hover:shadow-lg transition-shadow relative">
                            @if ($currentQuantity > 0)
                                <div class="absolute top-2 right-2 bg-blue-500 text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold">
                                    {{ $currentQuantity }}
                                </div>
                            @endif
                            ...
                            <div class="px-2 pb-2">
                                <form method="POST" action="{{ route('inventory.update') }}" class="space-y-2">
                                    @csrf
                                    ...
                                    <button type="submit" class="w-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded text-sm">
                                        Update
                                    </button>
                                </form>
                            </div>
